...
def traduire_vers_anglais(texte, langue_origine):
...
def summarize_en(texte):
    summ = TextSummarizer()
    summary_en =summ.summarize(texte)
    print("---------------")
    print("summarize_en()")
    print("[orig]: ", texte)
...
def generer_description(info):
    ...
    description_traduite = traduire_vers_anglais(description_originale, langue_origine)
    summary =  summarize_en(description_traduite)
...
def sauvegarder_donnees(data, fichier="resultats.json"):
    try:
        description_traduite = traduire_vers_anglais(data.get("description", ""), data.get("langue", ""))
        data["description_en"] = description_traduite  # Ajout de la traduction
...
def creer_description_entreprise(url):
    ...
    if contenu:
        ...
        sauvegarder_donnees(informations)  # Sauvegarde les informations analysées avec traduction
        return description
    else:
...
if __name__ == "__main__":
    url = "https://www.csfoy.ca/accueil"  # Remplace par l'URL de ton choix
